...
def load_aneel_linhas() -> dict:
    def fn():
        client = DadosGovClient()
        pkg = client.package_show("sistema-de-informacoes-georreferenciadas-do-setor-eletrico-sigel")
        res = client.pick_resource(pkg, format_hint="SHP", name_hint="linha transmiss")
        if not res:
            raise RuntimeError("Recurso SHP SIGEL não encontrado")
        content = client.download_resource(res, max_mb=300)
        shp = _extract_shapefile(content)
        if not shp:
            raise RuntimeError("SHP SIGEL não extraído")
        persisted = _load_shapefile_to_table(
            shp, AneelLinhaTransmissao, "aneel_linhas_transmissao",
            column_map={
                "Nome": "nome", "Tensao": "tensao_kv", "Operador": "operador",
                "Situacao": "situacao", "Extensao_k": "comprimento_km",
            },
        )
        return persisted, persisted, res.get("url", "")
    return _run_loader("aneel_linhas", "sistema-de-informacoes-georreferenciadas-do-setor-eletrico-sigel", fn)
